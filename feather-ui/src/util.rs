...
pub trait RcGraph {
    fn handle(&self, ctx: &mut RcGraphContext);
}

impl RcGraphContext {
    fn insert<T: stable_deref_trait::StableDeref>(&mut self, t: T)
    where
        T::Target: RcGraph,
    {
    }
...
impl<T: RcGraph> RcGraph for std::rc::Rc<T> {
    fn handle(&self, ctx: &mut RcGraphContext) {
        ctx.insert(self);
    }
}
