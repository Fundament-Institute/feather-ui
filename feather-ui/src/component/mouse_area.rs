...
#[derive(Debug, EnumVariantType, Dispatch, Clone, PartialEq)]
...
struct MouseAreaInternal<P1: SignalProvider<Item = f32> + ?Sized> {
...
impl<P1: SignalProvider<Item = f32> + ?Sized> MouseAreaInternal<P1> {
...
impl<P1: SignalProvider<Item = f32> + ?Sized>
    event::StateMachine<RawEvent, (PxRect, RelDim), MouseAreaEvent, MouseAreaState>
    for MouseAreaInternal<P1>
{
    fn update(
        &mut self,
        event: RawEvent,
        ...
        match event {
            ...
            RawEvent::MouseMove {
                ...
                self.state = Self::hover_state(all_buttons, self.hover);
                for i in 0..5 {
                    let deadzone = crate::sample_val(&self.deadzone);
...
impl<'a, T: leaf::Prop> MouseArea<T> {
    pub fn new(
        props: T,
        deadzone: Signal<impl SignalProvider<Item = f32> + ?Sized + 'static>,
    ) -> (
        Self,
        ...
        let (stream, output) = event::statemachine(
            MouseAreaInternal {
                lastdown: Default::default(),
                hover: false,
                deadzone,
